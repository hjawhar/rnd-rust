...
impl Market for MeteoraDlmmMarket {
    ...
    /// Real bin-walk quote when live pool bytes + bin arrays are available.
    ///
    /// Behavior change vs the old linear estimate: with a provider present,
    /// a missing bin array (swap larger than the fetched bin window) returns
    /// `Err` instead of a linear guess — the router skips the pool for that
    /// size.
    fn calculate_output_live_ex(
        ...
        let Some(provider) = accounts else {
            // No provider (CLI path): legacy linear estimate.
        ...
        // Provider present but no live pool bytes: active_id is
        // unverifiable and the cached snapshot can be months stale (a stale
        // active_id priced SOL 2.6x off through a linear estimate that also
        // ignores bin depth). Err -> router skips the pool until its
        // account streams in.
        let data = match pool_data {
            Some(d) if d.len() >= 712 => d,
            _ => {
                return Err(
                    "DLMM pool account bytes not in provider — pool unquotable until streamed"
                        .into(),
                );
            }
        };

        let pair = bin_math::DlmmPairState::parse(data)?;
        let lb_pair: Pubkey = self
            .pool_address
            .parse()
            .map_err(|_| "invalid pool address")?;

        // Market Buy = quote -> base. Physical: !flipped means quote = Y,
        // so Buy = Y -> X (swap_for_x, active_id walks up); Sell = X -> Y
        // (swap_for_y, walks down).
        let effective_direction = if self.flipped {
            match direction {
                SwapDirection::Buy => SwapDirection::Sell,
                SwapDirection::Sell => SwapDirection::Buy,
        ...
        let swap_for_y = effective_direction == SwapDirection::Sell;

        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_secs() as i64)
            .unwrap_or(pair.last_update_timestamp);

        bin_math::compute_swap_output(&pair, &lb_pair, amount_in, swap_for_y, provider, now)
    }
