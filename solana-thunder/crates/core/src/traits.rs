...
pub trait Market: Send + Sync {
    ...
    /// Calculate output using live on-chain data plus auxiliary accounts
    /// (tick arrays, bin arrays, fee configs) available via `accounts`.
    ///
    /// Default: ignores `accounts` and delegates to `calculate_output_live`,
    /// so every DEX crate compiles unchanged until it is upgraded to real
    /// curve math. Overrides may return `Err` when required auxiliary
    /// accounts (e.g. tick/bin arrays for the requested size) are missing
    /// from the provider — callers should treat that as "pool not quotable
    /// at this size" and skip it.
    fn calculate_output_live_ex(
        ...
        _accounts: Option<&dyn AccountDataProvider>,
    ) -> Result<u64, GenericError> {
