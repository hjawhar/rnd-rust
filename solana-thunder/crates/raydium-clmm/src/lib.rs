...
impl Market for RaydiumClmmMarket {
    ...
    /// Real curve math when auxiliary accounts are available.
    ///
    /// Behavior change vs the old linear estimate: with a provider present,
    /// a missing tick array (pool outside the cold-start prefetch, or a swap
    /// larger than the fetched tick-array window) returns `Err` instead of a
    /// linear guess — the router skips the pool for that size.
    fn calculate_output_live_ex(
        ...
        match (pool_data, accounts) {
            (Some(data), Some(provider)) if data.len() >= 1032 => self.calculate_output_walk(
                amount_in,
                direction,
                data,
                quote_vault_balance,
            ...
            // Provider present but no live pool bytes: current sqrt_price /
            // tick are unverifiable and the cached snapshot can be months
            // stale (and a linear estimate ignores bin/tick depth anyway).
            // Err -> router skips the pool until its account streams in.
            (_, Some(_)) => Err(
                "CLMM pool account bytes not in provider — pool unquotable until streamed"
                    .into(),
            ),
...
// ============================================================================
// Live tick-walk helper
// ============================================================================

impl RaydiumClmmMarket {
    /// Real tick-walk quote using live pool bytes + tick arrays + amm_config
    /// from the account provider. See `swap_math` for layout/rounding notes.
    ///
    /// Direction mapping: a Market-level `Buy` (spend quote for base) is,
    /// after un-flipping, physically "token_0 in, token_1 out", i.e.
    /// zero_for_one (price/tick decrease). This agrees with
    /// `compute_clmm_remaining_accounts`' `is_buy` semantics
    /// (is_buy == a_to_b == tick decreases).
    fn calculate_output_walk(
        &self,
        amount_in: u64,
        direction: SwapDirection,
        pool_data: &[u8],
        ...
        let state = swap_math::ClmmPoolState::parse(pool_data)?;
        if state.liquidity == 0 && state.tick_array_bitmap.iter().all(|w| *w == 0) {
            return Err("Pool has zero liquidity".into());
        }
        ...
        // Physical Buy = token_0 in -> zero_for_one (price down).
        let zero_for_one = physical_direction == SwapDirection::Buy;

        let pool_id: Pubkey = self
            .pool_address
            .parse()
            .map_err(|_| "invalid pool address")?;
        let out =
            swap_math::compute_swap_output(&state, &pool_id, amount_in, zero_for_one, provider)?;

        // Safety cap: never quote more than the destination vault holds.
        let (vault_0_balance, vault_1_balance) = if self.flipped {
            (base_vault_balance, quote_vault_balance)
        } else {
            (quote_vault_balance, base_vault_balance)
        };
        let cap = if zero_for_one { vault_1_balance } else { vault_0_balance };
