...
use crate::types::{LoadPhase, LoadProgress, SplitQuote};
...
fn print_help() {
    ...
    println!("  route <from> <to> <amount> <2-4> - Split-route across N pool-disjoint paths");
...
fn cmd_route(index: &PoolIndex, parts: &[&str]) {
    if parts.len() < 4 {
        println!("Usage: route <from_mint> <to_mint> <amount_raw> [splits]");
        return;
    }
    ...
    // Optional 5th arg: split routing ("splits"/"auto" -> 3 paths, or 2-4).
    let split_paths: Option<usize> = parts.get(4).and_then(|s| match *s {
        "splits" | "auto" => Some(3),
        other => other.parse::<usize>().ok().filter(|n| *n >= 2).map(|n| n.min(4)),
    });
    if let Some(max_paths) = split_paths {
        let cfg = SplitConfig {
            max_paths,
            ..Default::default()
        };
        match router.find_split_routes(from_mint, to_mint, amount, &cfg) {
            Ok(sq) if !sq.paths.is_empty() => print_split_quote(&sq, &from_mint, &to_mint),
            Ok(_) => println!("  No routes found"),
            Err(e) => println!("  Error finding split routes: {e}"),
        }
        return;
    }
...
/// Print the split allocation table plus the split-vs-single delta.
fn print_split_quote(sq: &SplitQuote, from_mint: &Pubkey, to_mint: &Pubkey) {
    println!("  Split allocation ({} path{}):", sq.paths.len(), if sq.paths.len() == 1 { "" } else { "s" });
    for (i, p) in sq.paths.iter().enumerate() {
        let hops_desc = p
            .route
            .hops
            .iter()
            .map(|h| format!("{}({})", h.dex_name, trunc(&h.pool_address)))
            .collect::<Vec<_>>()
            .join(" -> ");
        println!(
            "    Path {}: {:>6.2}% | {} -> {} | {}",
            i + 1,
            p.percent_bps as f64 / 100.0,
            fmt_amount(p.amount_in, from_mint),
            fmt_amount(p.route.output_amount, to_mint),
            hops_desc,
        );
    }
    println!("    Total output:  {}", fmt_amount(sq.total_output, to_mint));
    println!("    Single path:   {}", fmt_amount(sq.single_path_output, to_mint));
    if sq.single_path_output > 0 {
        let delta_bps = (sq.total_output as f64 - sq.single_path_output as f64)
            / sq.single_path_output as f64
            * 10_000.0;
        println!("    Split vs single: {delta_bps:+.2} bps");
    }
}
