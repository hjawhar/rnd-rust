...
/// One leg of a split quote: a route executed with a fraction of the input.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SplitPath {
    /// Absolute input amount routed down this path (raw units).
    pub amount_in: u64,
    /// Share of the total input in basis points (sums to 10_000 across paths).
    pub percent_bps: u16,
    /// The route, re-simulated exactly at `amount_in` (per-hop amounts real).
    pub route: Route,
}

/// A quote that splits the input across up to `max_paths` pool-disjoint routes.
///
/// Invariants: `Σ paths[i].amount_in == input_amount`,
/// `Σ paths[i].percent_bps == 10_000`, and `total_output >= single_path_output`
/// (a split that does not beat the best single route collapses to one path).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SplitQuote {
    /// Total input amount (raw units).
    pub input_amount: u64,
    /// Sum of all path outputs (raw units).
    pub total_output: u64,
    /// Paths ordered by `amount_in` descending (largest share first).
    pub paths: Vec<SplitPath>,
    /// Output of the best unsplit route at the full input amount.
    pub single_path_output: u64,
}
