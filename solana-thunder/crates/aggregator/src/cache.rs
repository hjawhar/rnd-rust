...
/// Extract the vault-share account keys for a Meteora DAMM V1 pool from its
/// serialized cached data: `(a_vault, b_vault, a_vault_lp, b_vault_lp)`.
/// The vault states are shared per-token accounts; the vault_lp token
/// accounts are per-pool. Quoting V1 needs all of them (plus each vault's
/// lp mint, whose pubkey lives inside the vault state account).
/// Returns None for non-V1 pools.
pub fn extract_damm_v1_vault_keys(
    cached_data: &[u8],
) -> Option<(Pubkey, Pubkey, Pubkey, Pubkey)> {
    let cached: CachedPool = bincode::deserialize(cached_data).ok()?;
    match cached {
        CachedPool::MeteoraDAMMV1 { pool, .. } => {
            Some((pool.a_vault, pool.b_vault, pool.a_vault_lp, pool.b_vault_lp))
        }
        _ => None,
    }
}
