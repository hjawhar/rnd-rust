...
impl Market for RaydiumAmmV4Market {
    ...
    fn calculate_output_live(
        ...
        // V4's live curve state IS the two vault balances (fees are static
        // pool fields), so quote from them whenever they're available —
        // pool account bytes are not required. Fall back to cached balances
        // only when the caller has no live balances at all.
        ...
        // NOTE: unlike `calculate_output` (which starts from PHYSICAL
        // `self.quote_balance`/`self.base_balance` and flips them into market
        // orientation), the caller passes vault balances that are ALREADY in
        // market orientation (balance of metadata().quote_vault/base_vault).
        // Re-flipping them here inverted the reserves for every flipped pool
        // (e.g. SOL/USDC 58oQChx..., where the pool's physical base is SOL):
        // quotes came out ~176x off. Use them as-is.
        let (quote_bal, base_bal) = (quote_vault_balance, base_vault_balance);
