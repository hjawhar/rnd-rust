...
export function getJwtToken() {
    return localStorage.getItem('vm_jwt');
}

export function getRefreshToken() {
    return localStorage.getItem('vm_refresh_token');
}
...
export function isAdmin(): boolean {
    ...
    const helper = new JwtHelperService();
    const decodedToken = helper.decodeToken(getJwtToken()!);
    if (isEmptyOrNull(decodedToken.group_id)) {
        return false;
    }
    return decodedToken.group_id === 1;
}
...
export function isCustomer(): boolean {
    ...
    return decodedToken.group_id === 3;
}
