...
pub async fn process_buy(task: ProcessBuy, nc: async_nats::Client) {
    let state = get_state();
    let market_pair: MarketPair = match serde_json::from_str(&task.market_pair) {
        Ok(mp) => mp,
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to deserialize market_pair: {}", e);
            return;
        }
    };
    ...
    // Phase 1.2: Pre-warm ATA cache for destination token account
    let user_address = match vm_solana::utils::helpers::str_to_pk(task.private_key.clone()) {
        Ok(kp) => kp.pubkey(),
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to parse private key: {}", e);
            return;
        }
    };
    ...
    // For BUY: SOL → Token, so we receive the quote mint (the token)
    let token_mint = match market.get_quote_mint() {
        Ok(mint) => mint,
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to get quote mint: {}", e);
            return;
        }
    };

    // Pre-warm destination ATA cache (detect Token vs Token-2022 for correct ATA)
    let token_program = state.fetch_token_program(&token_mint.to_string()).await
        .unwrap_or(spl_token::ID);
    let destination_ata = get_associated_token_address_with_program_id(&user_address, &token_mint, &token_program);
    let _ = state.check_ata_existence(&[destination_ata]).await;

    // Pre-warm token metadata cache
    let _ = state.fetch_token_info(&token_mint.to_string()).await;

    // Pre-warm quote vault balance cache (SOL side for buys)
    let quote_vault_balance = match market.get_quote_vault() {
        Ok(vault) => {
            state.get_vault_balance(&vault.to_string()).map(|b| b as f64 / 1e9)
        }
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to get quote vault: {}", e);
            None
        }
    };
    ...
    if let Some(max_impact_bps) = task.max_market_impact_bps {
        match calculate_buy_impact(&market_pair, task.value, quote_vault_balance) {
            Ok(impact_result) => {
                if let Err(e) = validate_impact(&impact_result, max_impact_bps) {
                    tracing::error!("[PROCESS_BUY] {}", e);

                    let msg = format!(
                        "Buy skipped for project {} - {}. Retrying next interval.",
                        task.project_id, e
                    );
                    let _ = nc
                        .publish(
            ...
            Err(e) => {
                tracing::error!("[PROCESS_BUY] Failed to calculate market impact: {}", e);
            }
        }
    }
    ...
    let mut ixs = match swap_result {
        Ok(ixs) => ixs,
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to build swap instructions: {}", e);
            return;
        }
    };
    ...
    let blockhash = match rpc_client.get_latest_blockhash().await {
        ...
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to fetch latest blockhash: {}", e);
            return;
        }
    };
    ...
    let message = match V0Message::try_compile(&kp.pubkey(), &ixs, &[], blockhash) {
        Ok(msg) => msg,
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to compile message: {}", e);
            return;
        }
    };
    ...
    let tx = match VersionedTransaction::try_new(VersionedMessage::V0(message), &[&kp]) {
        Ok(tx) => tx,
        Err(e) => {
            tracing::error!("[PROCESS_BUY] Failed to create transaction: {}", e);
            return;
        }
    };

    // Pre-flight Jito bundle simulation — skip if tx would fail on-chain
    if let Err(e) = simulate_jito_bundle(std::slice::from_ref(&tx), &["BUY"]).await {
        tracing::error!("[PROCESS_BUY] Jito simulation FAILED for project {}:\n{}", task.project_id, e);
        return;
    }
    tracing::info!("[PROCESS_BUY] Jito simulation OK for project {}", task.project_id);
    ...
    if let Err(e) = &jito_response {
        tracing::error!("[PROCESS_BUY] Jito bundle failed: {}", e);
    }
