...
use vm_data::models::task::EvmProcessBuy;
...
pub async fn process_buy(
    ...
    let token: CustomToken = match serde_json::from_str(&task.token_info) {
        Ok(t) => t,
        Err(e) => {
            tracing::error!("[EVM] Buy: failed to parse token_info for project {}: {}", task.project_id, e);
            return Err(e.into());
        }
    };
    let pool: CustomPool = match serde_json::from_str(&task.pool_info) {
        Ok(p) => p,
        Err(e) => {
            tracing::error!("[EVM] Buy: failed to parse pool_info for project {}: {}", task.project_id, e);
            return Err(e.into());
        }
    };

    let amount_in = f64_to_wei(task.value);
    let amount_out_minimum = if task.tokens > 0.0 {
        f64_to_token_units(task.tokens, token.decimals)
    } else {
        U256::from(0)
    };
