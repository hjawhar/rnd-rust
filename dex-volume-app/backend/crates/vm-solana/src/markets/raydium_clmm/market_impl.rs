//! Phase 3: Market trait implementation for Raydium CLMM
//!
//! This module implements the generic Market trait for Raydium Concentrated Liquidity Market Maker (CLMM).
//! CLMM uses tick-based pricing instead of constant product formula.
...
use crate::markets::raydium_clmm::models::raydium_clmm_pool::RaydiumCLMMPool;
...
impl RaydiumCLMMMarket {
    pub fn new(pool: RaydiumCLMMPool, market_pair: MarketPair) -> Self {
        Self {
            pool,
            market_pair,
            vault_0_balance: 0,
    ...
    fn calculate_clmm_output(&self, amount_in: u64, direction: SwapDirection) -> Result<u64, GenericError> {
        // Get current price
        ...
        let output = match direction {
            SwapDirection::Buy => {
                ...
                // output ≈ amount_in * price (adjusted for decimals)
...
impl Market for RaydiumCLMMMarket {
    ...
    fn build_swap_instruction_pure(
        ...
        use borsh::to_vec;
        use crate::utils::constants::{RAYDIUM_CLMM, TOKEN_PROGRAM, TOKEN_PROGRAM_2022, MEMO_PROGRAM_V2};
        use crate::markets::raydium_clmm::utils::tick_arrays::{
            compute_clmm_remaining_accounts, pda_array_bitmap_address,
        };
        ...
        let pair = Pubkey::from_str_const(&self.market_pair.pair);
        ...
        let ext_data = context.extra_accounts.get(&tick_array_bitmap.to_string());
        ...
        match direction {
            SwapDirection::Buy => {
                ...
                let wsol_pubkey = Pubkey::create_with_seed(&context.user, seed, &spl_token::id())?;

                let total_amount = TOKEN_ACCOUNT_RENT + args.amount_in;

                // Create temp WSOL account
                instructions.push(solana_system_interface::instruction::create_account_with_seed(
                    &context.user,
                    &wsol_pubkey,
                    &context.user,
                    seed,
                ...
                let remaining_accounts = compute_clmm_remaining_accounts(
                    &self.pool, &pair, true, ext_data.map(|v| v.as_slice()),
                )?;
            ...
            SwapDirection::Sell => {
                ...
                // Compute tick array remaining accounts (pure!)
                let remaining_accounts = compute_clmm_remaining_accounts(
                    &self.pool, &pair, false, ext_data.map(|v| v.as_slice()),
                )?;
                for remaining_account in remaining_accounts {
                    keys.push(AccountMeta::new(remaining_account, false));
                }
    ...
    fn required_accounts(
        ...
        use crate::markets::raydium_clmm::utils::tick_arrays::pda_array_bitmap_address;
...
// Implement ConcentratedLiquidity trait for CLMM
impl crate::markets::traits::ConcentratedLiquidity for RaydiumCLMMMarket {
