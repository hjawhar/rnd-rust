...
use crate::utils::constants::RAYDIUM_CLMM;
...
const EXTENSION_GROUP_SIZE: usize = 512; // bits per group (8 × u64)
const EXTENSION_NUM_GROUPS: usize = 14;

/// Check if a tick array is initialized using the bitmap extension account data.
/// Returns None if tick_array_index is within the in-pool bitmap range (±512).
fn is_tick_array_initialized_in_extension(extension_data: &[u8], tick_array_index: i32) -> bool {
    ...
    if tick_array_index > 0 {
        ...
        let word = u64::from_le_bytes(extension_data[data_offset..data_offset + 8].try_into().unwrap_or([0; 8]));
        word & (1u64 << bit_idx) != 0
    } else {
        ...
        // Negative bitmap starts after positive bitmap (14 groups × 8 words × 8 bytes)
...
///
/// This replaces the external HTTP API call that was previously used to fetch remaining accounts.
...
mod tests {
    ...
    fn make_test_pool(tick_current: i32, tick_spacing: u16, bitmap: [u64; 16]) -> RaydiumCLMMPool {
        RaydiumCLMMPool {
            ...
            status: 0,
            padding: [0; 7],
            reward_infos: std::array::from_fn(|_| crate::markets::raydium_clmm::models::raydium_clmm_pool::RewardInfo {
                reward_state: 0,
                open_time: 0,
                end_time: 0,
                last_update_time: 0,
