...
pub async fn add_user_req(
    ...
    let new_user = state.get_db().add_user(&new_user).await;
    let body = Json(json!({
        "data": new_user,
...
pub async fn whitelist_user_req(
    ...
    let user = state.get_db().get_user_by_id(user_id).await;
    if let Some(user) = user {
        ...
        let updated = state.get_db().update_user(&user_id, &new_user).await;
        let body = Json(json!({
            "data":  if updated {
